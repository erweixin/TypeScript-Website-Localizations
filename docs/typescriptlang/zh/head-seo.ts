export const headCopy = {
  head_playground_title:
    "TS Playground - 一个用于探索 TypeScript 和 JavaScript 的在线编辑器",
  head_playground_description:
    "可以通过 Playground 以安全且可共享的方式在线编写 TypeScript 和 JavaScript。",
  tsconfig_title: "TSConfig 参考 - 所有 TSConfig 选项的文档",
  tsconfig_description:
    "从 allowJs 到 useDefineForClassFields， TSConfig 的参考包含所有关于配置项目的有效的 TypeScript 编译器选项。",
  playground_example_prefix: "Playground 示例 - ",

  head_index_title: "拥有类型的 JavaScript，且适用于任何规模的工程",
  head_index_description: "拥有类型的 JavaScript，且适用于任何规模的工程",
}
